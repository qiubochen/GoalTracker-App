...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerItemDecoration(Context context) {
        ...
        // 此处设置了两只分别设置 时分 & 年月
        mPaint1 = new Paint();
        mPaint1.setColor(Color.parseColor("#2894FF"));
        mPaint1.setTextSize(30);

        mPaint2 = new Paint();
        mPaint2.setColor(Color.parseColor("#2894FF"));
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            // 设置文本起始坐标
            float Text_x = child.getLeft() - itemView_leftinterval * 5 / 6-2;
            float Text_y = upLine_bottom_y+10;

            List<Event>eventList=LitePal.where("done = ?","1").find(Event.class);
            Event event=eventList.get(index);
            if (event.getCompleteTime()!=null) {
                String[] time = DateUtil.getTimeLine(event.getCompleteTime());
                c.drawText(time[1], Text_x, Text_y, mPaint1);
                c.drawText(time[0], Text_x + 5, Text_y + 20, mPaint2);
            }
